...
void MainWindow::step_Limitation(){
    ...
    cursor.insertHtml( QString(
        ...
        "<br/><br/></p>"
    ).arg(limite).arg(nomMetropole).arg(siegesDroit));
    ...
    if( tooMuch == nullptr ){
        ...
        ).arg(tooMuch->nom).arg(tooMuch->totalSieges()).arg(siegesMetropole).arg(limite).arg(redist));
        ...
        for( int siege=1; siege<=redist; siege++){
            ...
            //---- Table des moyennes
            cursor.insertHtml( QString(
                "<h3>Nouvelles moyennes dans l'<u>hypothèse</u> de la réattribution du siège N° %1 :</h3>"
            ).arg(siege));
            ...
            tooMuch->sieges4 -= 1;
            cursor.insertHtml( QString(
                "<h3>&nbsp; &nbsp; &rArr; Le siège N° %1 est donc réattribué à %2 "
                "pour sa moyenne de %3 hab/siège</h3><br/><br/>"
            ).arg(siege).arg(winner->nom).arg(winner->moyenne));
        ...
        cursor.insertHtml( QString(
            "<h2>RESUME de la réattribution à %1 communes (à la plus forte moyenne) "
            "de %2 siéges pris à %3 :</h2>"
        ).arg(nbrWinners).arg(redist).arg(tooMuch->nom));
    ...
    //---- L5211-6-1 IV 4°
    //---- Vérification que conseillers métropolitains <= conseillers municipaux

    bool fatalErr = false;
    for( const auto * const commune : std::as_const(parPopulation) ){
        if( commune->totalSieges() > commune->cm ){
            cursor.insertHtml( QString(
            "<p style=\"font-size:12pt; color:red;\"><b>ATTENTION</b> : "
    ...
    if( fatalErr ){
        cursor.insertHtml( QString(
        "<p style=\"font-size:12pt; color:red;\"><br/>"
        ...
        cursor.insertHtml( QString(
            ...
            "de %1 que ce qu'elle a de conseillers municipaux."
            "<br/></p>"
        ).arg(nomMetropole));
    }
    ...
    cursor.insertHtml( QString(
        "<hr><h1>Répartition des %1 sieges de plein droit :</h1>"
        "<p style=\"font-size:12pt;\">"
