...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), canPrint(false), ui(new Ui::MainWindow) {
    ui->setupUi(this);
...
void MainWindow::on_comboBox_metropole_activated(int index) {
    if (index == 0)
        return; // c'est la premiere ligne avec le message "Selectionnez ..."
    const auto ville    = ui->comboBox_metropole->currentText();
    const auto dataFile = getDataPath().append("/2025/").append(ville).append(".csv");
    processDataFile(dataFile,';');  // Nos fichiers csvcutilisent toujousr le ';'
}
...
void MainWindow::on_pushButton_load_csv_clicked() {
    ...
    const QString filename
        = QFileDialog::getOpenFileName(this, "Selectionner le fichier des communes et populations", loadFromPath,
    ...
    // prend le premier caractère de l'intitulé selectionné comme séparateur
    const char separateur = *(ui->comboBox_Separateur->currentText().toLocal8Bit().constData());

    processDataFile(filename,separateur);
}

void MainWindow::processDataFile(const QString& filename, char separateur) {
    communes = loadCommunes(filename, separateur);
    if (communes.isEmpty()) {
    ...
    if (communes[0].population < 0) {
        ...
        msgErrorBox(
            QString("Fichier: %1\n%2\nAvec %3 comme séparateur.").arg(filename, communes[0].nom).arg(separateur));
        return;
    }
...
void MainWindow::setCell(QTextTable*             table,
                         int                     line,
                         int                     col,
                         const QString&          text,
...
void MainWindow::setHtmlCell(QTextTable*             table,
                             int                     line,
                             int                     col,
                             const QString&          html,
                             const QTextBlockFormat& blkFmt) {
...
QTextCharFormat MainWindow::defaultCharFormat, MainWindow::boldCharFormat;

QTextBlockFormat MainWindow::defaultBlockFormat, MainWindow::rightBlockFormat, MainWindow::leftBlockFormat;

QTextTableFormat MainWindow::defaultTableFormat, MainWindow::tblFormat;
