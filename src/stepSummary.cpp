...
void MainWindow::showSummary(QTextCursor cursor, const QList<InfoCommune*>& sortBy) {
    ...
    int sumPopulation = 0, sumSieges1 = 0, sumSieges2 = 0, sumSieges3 = 0, sumSieges4 = 0, sumSieges5 = 0,
        sumSieges = 0;
...
void MainWindow::step_Summary() {
    ...
    p = "<p style=\"font-size:12pt;\">Base: %1<br/>Plein droit: %2<br/>Maxi: %3</p>";
    setHtmlCell(table, 3, 1, QString(p).arg(siegesBase).arg(siegesDroit).arg(siegesMax));
    ...
    cursor.insertHtml(QString(p).arg(modeStr));

    // calcule l'état final de chaque commune
