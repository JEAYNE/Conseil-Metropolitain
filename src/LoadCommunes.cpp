...
Communes loadCommunes(const QString& filename, const char delim){
    ...
    if( ! file.open(QIODevice::ReadOnly) ){
        data.push_front(InfoCommune(QString("Ne peut pas ouvrir le fichier %1\nERREUR %2: %3")
                                            .arg(filename)
                                            .arg(file.error())
    ...
    while( ! stream.atEnd() ){
        ...
        if( fields.size() != 2 ){
            data.push_front(InfoCommune(QString("ligne %1: Nombre de champs invalide. 2 attendus mais %2 trouvés.")
                                                .arg(lineCount)
                                                .arg(fields.size())
                                               ));
            return data;
        ...
        if( fields[0].length()==0 ){
            data.push_front(InfoCommune(QString("ligne %1: Le premier champ 'Nom' est vide.").arg(lineCount)));
            return data;
        }
        const auto &nom = fields[0];
        for (const auto& item : std::as_const(data)) {
            if( nom == item.nom ){
                data.push_front(InfoCommune(QString("ligne %1: Ce nom de commune est dupliqué.").arg(lineCount)));
                return data;
            }
        }
        ...
        if( fields[1].length()==0 ){
            data.push_front(InfoCommune(QString("ligne %1: Le second champ 'Population' est vide.").arg(lineCount)));
            return data;
        }
        bool ok;
        const auto population = fields[1].toInt(&ok);
        if( (!ok) || (population<50) || (population>3'000'000)){
            data.push_front(InfoCommune(QString("ligne %1: Le second champ 'Population' n'a pas une valeur valide.").arg(lineCount)));
            return data;
        }
